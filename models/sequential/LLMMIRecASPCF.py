# -*- coding: UTF-8 -*-
"""
LLMMIRecASPCF — Chapter 3: ASPCF + relation preservation (no prototype query).

Fixed: ASPCF item encoder, basic gate, semantic_rank, relation loss,
       learnable interest query.
"""
...
class LLMMIRecASPCF(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    ...
    def __init__(self, args, corpus):
        ...
        logging.info(f"[ASPCF] initialized: item_encoder={self.item_encoder_mode} "
                     f"K={self.K} semantic_rank={self.semantic_rank} "
                     f"lambda_relation={self.lambda_relation} gate={self.aspcf_gate_mode}")
        logging.info(f"[ASPCF] #params: {self.count_variables()}")
    ...
    def forward(self, feed_dict, return_intermediate=False):
        ...
        # 4. Multi-interest extraction (learnable queries only)
        ...
        if not self._first_batch_checked:
            ...
            logging.info("[ASPCF] First-batch NaN/Inf check passed.")
