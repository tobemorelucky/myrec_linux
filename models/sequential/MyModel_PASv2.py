...
Replaceable implementation for the paper/code line:
  PoMRec backbone + LLM semantic alignment/injection + PAS-v2 soft evidence router + TIC/IPD.

PAS-v2 = Period-aware Semantic-Collaborative Soft Evidence Router.
Important properties:
  1) History-only second module: PAS-v2 never uses target positive/negative items.
  ...
  5) Optional legacy LGD can be kept before PAS-v2 as a stable global history denoising layer.
"""
...
class PeriodAwareSampler(nn.Module):
    ...
    Different from the previous PAS hard top-k sampler, PAS-v2 does NOT delete
    ...
    def __init__(
        ...
        # hard/use_gumbel are accepted for script compatibility, but PAS-v2 uses soft routing.
        self.hard = int(hard)
    ...
    def forward(
        ...
        # Encourage retained high-score evidence to be semantically-collaboratively consistent.
        # Detach cosine to regularize router scores only.
        cos_detach = cos_sc.detach()
        selected_prob = (prob_e + prob_a).clamp(0.0, 1.0) * valid
        L_sc = -((selected_prob * cos_detach).sum() / (selected_prob.sum() + eps))
...
class MyModel(SequentialModel):
    ...
    def parse_model_args(parser):
        ...
        # PAS-v2 soft attention-prior strength
    ...
    def __init__(self, args, corpus):
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModel-PASv2] Warm-start from {self.init_ckpt} (strict={bool(self.init_strict)})")
        else:
            logging.info("[MyModel-PASv2] Train from scratch")
    ...
    def forward(self, feed_dict):
        ...
        # Optional stable global LGD before PAS-v2.
        ...
        if self.use_pas:
            if not self.use_llmemb:
                raise RuntimeError("PAS-v2 requires use_llmemb=1 because semantic-CF router features are used.")
        ...
        # PAS-v2 auxiliary losses (history-only; no target-conditioned supervision)
    ...
    def loss(self, out_dict: dict):
        ...
        # PAS-v2 router auxiliary losses, warm-started.
