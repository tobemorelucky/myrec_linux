# -*- coding: UTF-8 -*-
"""
MyModelTIRL: PoMRec backbone + LLM + IPD + TIRL.

TIRL: Target-routed Interest-level Ranking Learning.
  - Training-only auxiliary ranking loss at the interest level.
  - For each sample, selects the most relevant interest to the positive item,
    then applies BPR between that interest's positive score and its negative scores.
  - Does NOT change the final prediction — only adds a loss.
  - No inference cost increase, no offline preprocessing, no codebook.

Modules:
  3.1 LLM Semantic Alignment & Controllable Injection
  3.2 TIRL: Target-routed Interest-level Ranking Learning (auxiliary loss)
...
# =========================
# MyModelTIRL: PoMRec + LLM + TIRL + IPD
# =========================
class MyModelTIRL(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    extra_log_args = [
        "emb_size", "lr", "use_emile", "lambda_ipd",
        "use_tirl", "lambda_tirl", "tirl_mode",
    ]
    ...
    def parse_model_args(parser):
        ...
        # ---- TIRL: Target-routed Interest-level Ranking Learning ----
        parser.add_argument("--use_tirl", type=int, default=1,
                            help="1: enable target-routed interest-level ranking loss")
        parser.add_argument("--lambda_tirl", type=float, default=0.01,
                            help="weight for TIRL loss")
        parser.add_argument("--tirl_warmup_steps", type=int, default=5000,
                            help="warmup steps for TIRL loss")
        parser.add_argument("--tirl_mode", type=str, default="selected",
                            help="routing mode: selected (argmax) or max")
        parser.add_argument("--tirl_detach_route", type=int, default=1,
                            help="1: detach the interest selection index")
        parser.add_argument("--tirl_neg_reduce", type=str, default="mean",
                            help="negative reduction: mean or max")
    ...
    def __init__(self, args, corpus):
        ...
        # TIRL
        self.use_tirl = int(getattr(args, "use_tirl", 1))
        self.lambda_tirl = float(getattr(args, "lambda_tirl", 0.01))
        self.tirl_warmup_steps = int(getattr(args, "tirl_warmup_steps", 5000))
        self.tirl_mode = str(getattr(args, "tirl_mode", "selected"))
        self.tirl_detach_route = int(getattr(args, "tirl_detach_route", 1))
        self.tirl_neg_reduce = str(getattr(args, "tirl_neg_reduce", "mean"))
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelTIRL] Warm-start from {self.init_ckpt}")
        else:
            logging.info("[MyModelTIRL] Train from scratch")
        self.global_step = 0
    ...
    def _tirl_w(self):
        if self.tirl_warmup_steps <= 0:
            return 1.0
        t = min(self.global_step, self.tirl_warmup_steps)
        return t / float(self.tirl_warmup_steps)
    ...
    def _cos_dist(a, b, eps=1e-8):
        return 1.0 - MyModelTIRL._cos_sim(a, b, eps=eps)
    ...
    # TIRL loss
    # -------------------------
    def compute_tirl_loss(self, out_dict):
        """
        Target-routed interest-level ranking loss.

        For each sample, identifies the interest most relevant to the positive item,
        then applies BPR between that interest's positive score and negative scores.
        """
        interest_vectors = out_dict["tirl_interest_vectors"]  # (B, K, D)
        item_vectors = out_dict["tirl_item_vectors"]          # (B, N, D)

        if item_vectors.size(1) <= 1:
            return torch.zeros([], device=item_vectors.device)

        # Interest-level scores: (B, K, N)
        interest_scores = torch.einsum("bkd,bnd->bkn", interest_vectors, item_vectors)

        pos_scores = interest_scores[:, :, 0]     # (B, K)
        neg_scores = interest_scores[:, :, 1:]    # (B, K, N-1)

        if self.tirl_mode == "selected":
            route_idx = pos_scores.argmax(dim=1)  # (B,)
            if self.tirl_detach_route:
                route_idx = route_idx.detach()
            ...
            if self.tirl_neg_reduce == "max":
                hard_neg = selected_neg.max(dim=1).values  # (B,)
                loss_tirl = -F.logsigmoid(selected_pos - hard_neg).mean()
            else:  # mean
                loss_tirl = -F.logsigmoid(selected_pos[:, None] - selected_neg).mean()

        else:  # tirl_mode == "max"
            pos_mi = pos_scores.max(dim=1).values   # (B,)
            neg_mi = neg_scores.max(dim=1).values   # (B, N-1)
            loss_tirl = -F.logsigmoid(pos_mi[:, None] - neg_mi).mean()
    ...
    def forward(self, feed_dict):
        ...
        # TIRL stash
        if self.use_tirl:
            out_dict["tirl_interest_vectors"] = interest_vectors
            out_dict["tirl_item_vectors"] = i_vectors
    ...
    def loss(self, out_dict):
        ...
        # TIRL: target-routed interest-level ranking loss
        if self.use_tirl and ("tirl_interest_vectors" in out_dict):
            loss_tirl = self.compute_tirl_loss(out_dict)
            loss = loss + self._tirl_w() * self.lambda_tirl * loss_tirl
            out_dict["loss_tirl"] = loss_tirl.detach()

            if self.training and self.global_step % 1000 == 0:
                interest_vectors = out_dict["tirl_interest_vectors"]
                item_vectors = out_dict["tirl_item_vectors"]
                if item_vectors.size(1) > 1:
                    scores = torch.einsum("bkd,bnd->bkn", interest_vectors, item_vectors)
                    pos_m = scores[:, :, 0].max(dim=1).values.mean()
                    neg_m = scores[:, :, 1:].max(dim=1).values.mean()
                else:
                    pos_m = torch.zeros([]); neg_m = torch.zeros([])
                logging.info(
                    f"[TIRL] step={self.global_step} "
                    f"w={self._tirl_w():.4f} "
                    f"lambda={self.lambda_tirl:.4f} "
                    f"loss_tirl={loss_tirl.item():.6f} "
                    f"mode={self.tirl_mode} "
                    f"neg_reduce={self.tirl_neg_reduce} "
                    f"selected_pos_mean={pos_m.item():.4f} "
                    f"selected_neg_mean={neg_m.item():.4f}"
                )
