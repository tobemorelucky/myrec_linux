...
# =========================
# MyModelLLMIPD: PoMRec + LLM + IPD (clean single-pass)
# =========================
class MyModelLLMIPD(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    ...
    def __init__(self, args, corpus):
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelLLMIPD] Warm-start from {self.init_ckpt}")
        else:
            logging.info("[MyModelLLMIPD] Train from scratch")
        self.global_step = 0
    ...
    def _cos_dist(a, b, eps=1e-8):
        return 1.0 - MyModelLLMIPD._cos_sim(a, b, eps=eps)
