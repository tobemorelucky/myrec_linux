# -*- coding: UTF-8 -*-
"""
MyModelV4: Clean re-submission main model.

PoMRec backbone + 3 modules:
  3.1 LLM Semantic Alignment & Controllable Injection
  3.2 DSIP: Dynamic Semantic Interest Prompting
...
class MultiInterestExtractor(nn.Module):
    ...
    def forward(self, history, lengths, dyn_prompts=None):
        """
        Args:
            history: (B, L)
            lengths: (B,)
            dyn_prompts: (B, K, D) optional dynamic semantic prompt tokens
        Returns:
        ...
        # Base mask: history + static prompts
        ...
        # Append dynamic semantic prompts (no position embedding needed)
        if dyn_prompts is not None:
            K_dyn = dyn_prompts.size(1)
            his_vectors_prompt1 = torch.cat([his_vectors_prompt1, dyn_prompts], dim=1)
            valid_his_ext = torch.cat([valid_his_ext, torch.ones([B, K_dyn], device=device)], dim=1)
        ...
        # Also append dyn_prompts to aggregator input
        if dyn_prompts is not None:
            his_vectors_prompt2 = torch.cat([his_vectors_prompt2, dyn_prompts], dim=1)
...
# =========================
# MyModelV4: PoMRec + LLMAlign + DSIP + IPD
# =========================
class MyModelV4(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    ...
    def parse_model_args(parser):
        ...
        # ---- DSIP: Dynamic Semantic Interest Prompting ----
        parser.add_argument("--use_dsip", type=int, default=1,
                            help="1: enable dynamic semantic interest prompting")
        parser.add_argument("--dsip_scale", type=float, default=0.1,
                            help="scale of dynamic semantic prompt tokens")
        parser.add_argument("--dsip_query_init", type=str, default="random",
                            help="initialization of semantic interest queries")
        parser.add_argument("--dsip_detach_sem", type=int, default=1,
                            help="1: detach adapted semantic history when building dynamic prompts")
        parser.add_argument("--dsip_norm", type=int, default=1,
                            help="1: layernorm dynamic semantic prompts")
        parser.add_argument("--dsip_dropout", type=float, default=0.0,
                            help="dropout on dynamic semantic prompts")
    ...
    def __init__(self, args, corpus):
        ...
        # dsip
        self.use_dsip = int(getattr(args, "use_dsip", 1))
        self.dsip_scale = float(getattr(args, "dsip_scale", 0.1))
        self.dsip_detach_sem = int(getattr(args, "dsip_detach_sem", 1))
        self.dsip_norm = int(getattr(args, "dsip_norm", 1))
        self.dsip_dropout_p = float(getattr(args, "dsip_dropout", 0.0))

        self.dsip_queries = nn.Parameter(torch.randn(self.K, self.emb_size) * 0.02)
        self.dsip_ln = nn.LayerNorm(self.emb_size)
        self.dsip_dropout = nn.Dropout(self.dsip_dropout_p)
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelV4] Warm-start from {self.init_ckpt} (strict={bool(self.init_strict)})")
        else:
            logging.info("[MyModelV4] Train from scratch")
    ...
    def _cos_dist(a: torch.Tensor, b: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        return 1.0 - MyModelV4._cos_sim(a, b, eps=eps)
    ...
    # -------------------------
    # DSIP: compute dynamic semantic interest prompts
    # -------------------------
    def compute_dsip_prompts(self, history):
        """
        Build K dynamic semantic interest prototypes via attention over
        adapted LLM semantic embeddings of user history items.
        ...
        Returns:
            dyn_prompts: (B, K, D)
            aux: dict with "dsip_attn" (B, K, L), "dsip_prompt_norm"
        """
        hist_sem = self.interest_extractor.get_llm_emb(history)  # (B, L, D)

        if self.dsip_detach_sem:
            hist_sem = hist_sem.detach()

        valid = (history > 0)  # (B, L)

        # Attention: K learnable queries over history semantic embeddings
        q = F.normalize(self.dsip_queries, dim=-1)          # (K, D)
        h = F.normalize(hist_sem, dim=-1)                   # (B, L, D)

        score = torch.einsum("kd,bld->bkl", q, h) / math.sqrt(self.emb_size)
        score = score.masked_fill(~valid[:, None, :], -1e9)

        attn = torch.softmax(score, dim=-1)                 # (B, K, L)
        dyn_prompts = torch.matmul(attn, hist_sem)          # (B, K, D)

        if self.dsip_norm:
            dyn_prompts = self.dsip_ln(dyn_prompts)

        dyn_prompts = self.dsip_dropout(dyn_prompts)
        dyn_prompts = self.dsip_scale * dyn_prompts

        aux = {
            "dsip_attn": attn.detach(),
            "dsip_prompt_norm": dyn_prompts.detach().norm(dim=-1).mean(),
        }

        return dyn_prompts, aux
    ...
    def forward(self, feed_dict):
        ...
        # DSIP: build dynamic semantic prompts from history
        dyn_prompts = None
        dsip_aux = None
        if self.use_dsip and self.use_llmemb:
            dyn_prompts, dsip_aux = self.compute_dsip_prompts(history)

        interest_vectors, distri_vectors = self.interest_extractor(
            history, lengths,
            dyn_prompts=dyn_prompts,
        )
        ...
        # DSIP debug log
        if self.use_dsip and dsip_aux is not None and self.training and self.global_step % 1000 == 0:
            attn = dsip_aux["dsip_attn"]  # (B, K, L)
            entropy = -(attn * torch.log(attn + 1e-8)).sum(dim=-1)  # (B, K)
            logging.info(
                f"[DSIP] step={self.global_step} "
                f"scale={self.dsip_scale:.3f} "
                f"prompt_norm={dsip_aux['dsip_prompt_norm'].item():.4f} "
                f"attn_entropy={entropy.mean().item():.4f}"
            )
