# -*- coding: UTF-8 -*-
"""
MyModelSCIL: PoMRec backbone + LLM + IPD + SCIL.

SCIL: Semantic Codebook-guided Interest Learning.
  - Offline KMeans clusters LLM item embeddings into a semantic codebook.
  - During training, the user representation is supervised to predict the
    positive item's semantic cluster via a lightweight classification head.
  - Forward recommendation path is unchanged.
  - No extra modules in the recommendation forward pass.

Modules:
  3.1 LLM Semantic Alignment & Controllable Injection
  3.2 SCIL: Semantic Codebook-guided Interest Learning (auxiliary loss only)
...
# =========================
# MyModelSCIL: PoMRec + LLM + SCIL + IPD
# =========================
class MyModelSCIL(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    extra_log_args = [
        "emb_size", "lr", "use_emile", "lambda_ipd",
        "use_scil", "lambda_scil", "scil_num_clusters",
    ]
    ...
    def parse_model_args(parser):
        ...
        # ---- SCIL: Semantic Codebook-guided Interest Learning ----
        parser.add_argument("--use_scil", type=int, default=1,
                            help="1: enable semantic codebook-guided interest learning")
        parser.add_argument("--lambda_scil", type=float, default=0.003,
                            help="weight for SCIL loss")
        parser.add_argument("--scil_warmup_steps", type=int, default=5000,
                            help="warmup steps for SCIL loss")
        parser.add_argument("--scil_codebook_path", type=str, default="",
                            help="path to semantic codebook pkl")
        parser.add_argument("--scil_num_clusters", type=int, default=128,
                            help="number of semantic clusters (fallback)")
        parser.add_argument("--scil_head_hidden", type=int, default=0,
                            help="hidden dim for SCIL head; 0 = linear")
        parser.add_argument("--scil_detach_user", type=int, default=0,
                            help="1: detach user vector before SCIL head")
    ...
    def __init__(self, args, corpus):
        ...
        # SCIL
        self.use_scil = int(getattr(args, "use_scil", 1))
        self.lambda_scil = float(getattr(args, "lambda_scil", 0.003))
        self.scil_warmup_steps = int(getattr(args, "scil_warmup_steps", 5000))
        self.scil_codebook_path = getattr(args, "scil_codebook_path", "")
        self.scil_num_clusters = int(getattr(args, "scil_num_clusters", 128))
        self.scil_head_hidden = int(getattr(args, "scil_head_hidden", 0))
        self.scil_detach_user = int(getattr(args, "scil_detach_user", 0))

        if self.use_scil:
            if not self.scil_codebook_path:
                raise ValueError("use_scil=1 requires --scil_codebook_path")
            cb = pickle.load(open(self.scil_codebook_path, "rb"))
            item2cluster = cb["item2cluster"]
            if isinstance(item2cluster, np.ndarray):
                item2cluster = torch.from_numpy(item2cluster).long()
            else:
                item2cluster = torch.tensor(item2cluster, dtype=torch.long)
            self.register_buffer("item2cluster", item2cluster, persistent=False)
            self.scil_num_clusters = cb.get("num_clusters", self.scil_num_clusters)

            # Semantic intent prediction head
            if self.scil_head_hidden <= 0:
                self.scil_head = nn.Linear(self.emb_size, self.scil_num_clusters)
            else:
                self.scil_head = nn.Sequential(
                    nn.Linear(self.emb_size, self.scil_head_hidden),
                    nn.ReLU(),
                    nn.Linear(self.scil_head_hidden, self.scil_num_clusters),
                )
            logging.info(f"[SCIL] Loaded codebook: {self.scil_codebook_path}, clusters={self.scil_num_clusters}")
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelSCIL] Warm-start from {self.init_ckpt}")
        else:
            logging.info("[MyModelSCIL] Train from scratch")
        self.global_step = 0
    ...
    def _scil_w(self):
        if self.scil_warmup_steps <= 0:
            return 1.0
        t = min(self.global_step, self.scil_warmup_steps)
        return t / float(self.scil_warmup_steps)
    ...
    def _cos_dist(a, b, eps=1e-8):
        return 1.0 - MyModelSCIL._cos_sim(a, b, eps=eps)
    ...
    # =========================
    # forward (single-pass, unchanged rec path)
    # =========================
    def forward(self, feed_dict):
        ...
        # SCIL stash: user vector + positive item ids for auxiliary cluster prediction
        if self.use_scil:
            out_dict["scil_user_vector"] = u_base
            out_dict["scil_pos_ids"] = i_ids[:, 0]
    ...
    def loss(self, out_dict):
        ...
        # SCIL: semantic codebook-guided interest learning
        if self.use_scil and ("scil_user_vector" in out_dict):
            user_vec = out_dict["scil_user_vector"]
            pos_ids = out_dict["scil_pos_ids"]

            if self.scil_detach_user:
                user_vec = user_vec.detach()

            target_cluster = self.item2cluster[pos_ids]  # (B,)
            valid = target_cluster >= 0

            if valid.any():
                logits = self.scil_head(user_vec[valid])
                loss_scil = F.cross_entropy(logits, target_cluster[valid])
                loss = loss + self._scil_w() * self.lambda_scil * loss_scil
                out_dict["loss_scil"] = loss_scil.detach()

                if self.training and self.global_step % 1000 == 0:
                    head_norm = sum(p.norm() for p in self.scil_head.parameters()).item()
                    logging.info(
                        f"[SCIL] step={self.global_step} "
                        f"w={self._scil_w():.4f} "
                        f"lambda={self.lambda_scil:.6f} "
                        f"loss_scil={loss_scil.item():.6f} "
                        f"num_clusters={self.scil_num_clusters} "
                        f"target_unique={target_cluster[valid].unique().numel()} "
                        f"head_norm={head_norm:.4f}"
                    )
