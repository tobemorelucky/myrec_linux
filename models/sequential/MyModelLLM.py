# -*- coding: UTF-8 -*-
"""
MyModelLLM: Clean ablation — PoMRec backbone + LLM semantic enhancement only.

Modules:
  3.1 LLM Semantic Alignment & Controllable Injection
  (no IPD, no LGD, no SSID, no DSPC, no SICR)
"""
...
# =========================
# MyModelLLM: PoMRec + LLM (clean single-pass)
# =========================
class MyModelLLM(SequentialModel):
    reader = "SeqReader"
    runner = "BaseRunner"
    ...
    def __init__(self, args, corpus):
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelLLM] Warm-start from {self.init_ckpt}")
        else:
            logging.info("[MyModelLLM] Train from scratch")
        self.global_step = 0
