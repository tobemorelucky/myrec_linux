# -*- coding: UTF-8 -*-
"""
MyModelCIRF: PoMRec backbone + LLM + IPD + CIRF.

CIRF: Candidate-aware Interest Residual Fusion.
  - Lightweight forward scoring module: adds a gated residual from
    candidate-aware interest routing to the base user-vector prediction.
  - No auxiliary loss, no new parameters beyond gate thresholds.
  - Prediction = base + eta * gate * (route - base).

Modules:
  3.1 LLM Semantic Alignment & Controllable Injection
  3.2 CIRF: Candidate-aware Interest Residual Fusion
...
# ========================= MyModelCIRF =========================
class MyModelCIRF(SequentialModel):
    reader = "SeqReader"; runner = "BaseRunner"
    extra_log_args = ["emb_size", "lr", "use_emile", "lambda_ipd",
                      "use_cirf", "cirf_eta", "cirf_route_temp", "cirf_gate_type"]
    ...
    def parse_model_args(parser):
        ...
        # CIRF
        parser.add_argument("--use_cirf", type=int, default=1)
        parser.add_argument("--cirf_eta", type=float, default=0.05)
        parser.add_argument("--cirf_route_temp", type=float, default=0.2)
        parser.add_argument("--cirf_route_norm", type=int, default=1)
        parser.add_argument("--cirf_gate_type", type=str, default="margin")
        parser.add_argument("--cirf_conf_threshold", type=float, default=0.45)
        parser.add_argument("--cirf_margin_threshold", type=float, default=0.05)
        parser.add_argument("--cirf_gate_mode", type=str, default="linear")
        parser.add_argument("--cirf_gate_temp", type=float, default=0.05)
        parser.add_argument("--cirf_detach_route", type=int, default=1)
        parser.add_argument("--cirf_min_eta", type=float, default=0.0)
    ...
    def __init__(self, args, corpus):
        ...
        # CIRF
        self.use_cirf = int(getattr(args, "use_cirf", 1))
        self.cirf_eta = float(getattr(args, "cirf_eta", 0.05))
        self.cirf_eta = max(0.0, min(1.0, self.cirf_eta))
        self.cirf_route_temp = float(getattr(args, "cirf_route_temp", 0.2))
        self.cirf_route_norm = int(getattr(args, "cirf_route_norm", 1))
        self.cirf_gate_type = str(getattr(args, "cirf_gate_type", "margin"))
        self.cirf_conf_threshold = float(getattr(args, "cirf_conf_threshold", 0.45))
        self.cirf_margin_threshold = float(getattr(args, "cirf_margin_threshold", 0.05))
        self.cirf_gate_mode = str(getattr(args, "cirf_gate_mode", "linear"))
        self.cirf_gate_temp = float(getattr(args, "cirf_gate_temp", 0.05))
        self.cirf_detach_route = int(getattr(args, "cirf_detach_route", 1))
        self.cirf_min_eta = float(getattr(args, "cirf_min_eta", 0.0)); self.cirf_min_eta = max(0.0, min(1.0, self.cirf_min_eta))
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModelCIRF] Warm-start from {self.init_ckpt}")
        else: logging.info("[MyModelCIRF] Train from scratch")
        self.global_step = 0
    ...
    @staticmethod
    def _cos_dist(a, b, eps=1e-8): return 1.0 - MyModelCIRF._cos_sim(a, b, eps=eps)
    @staticmethod
    ...
    def forward(self, feed_dict):
        ...
        prediction_base = (u_base[:, None, :] * i_vectors).sum(dim=-1)
        batch_size = interest_vectors.size(0)

        if self.use_cirf:
            interest_scores = torch.einsum("bkd,bnd->bkn", interest_vectors, i_vectors)
            if self.cirf_route_norm:
                ri = F.normalize(interest_vectors, dim=-1); rj = F.normalize(i_vectors, dim=-1)
                route_scores = torch.einsum("bkd,bnd->bkn", ri, rj)
            else: route_scores = interest_scores
            route_prob = F.softmax(route_scores / self.cirf_route_temp, dim=1)
            if self.cirf_detach_route: route_prob = route_prob.detach()
            prediction_route = (route_prob * interest_scores).sum(dim=1)
            conf = route_prob.max(dim=1).values
            top2 = torch.topk(route_prob, k=min(2, route_prob.size(1)), dim=1).values
            margin = top2[:, 0, :] - top2[:, 1, :] if top2.size(1) >= 2 else conf
            if self.cirf_gate_type == "none":
                gate = torch.ones_like(prediction_base)
            else:
                base_g = conf if self.cirf_gate_type == "conf" else margin
                thr = self.cirf_conf_threshold if self.cirf_gate_type == "conf" else self.cirf_margin_threshold
                gate = torch.sigmoid((base_g - thr) / self.cirf_gate_temp) if self.cirf_gate_mode == "sigmoid" \
                    else ((base_g - thr) / (1.0 - thr)).clamp(min=0.0, max=1.0)
            gate = gate.detach()
            eta_eff = self.cirf_min_eta + self.cirf_eta * gate
            prediction = prediction_base + eta_eff * (prediction_route - prediction_base)
        else:
        ...
        if self.use_cirf and self.training and self.global_step % 1000 == 0:
            logging.info(
                f"[CIRF] step={self.global_step} eta={self.cirf_eta:.4f} route_temp={self.cirf_route_temp:.3f} "
                f"gate_type={self.cirf_gate_type} base_mean={prediction_base.mean().item():.4f} "
                f"route_mean={prediction_route.mean().item():.4f} pred_mean={prediction.mean().item():.4f} "
                f"gate_mean={gate.mean().item():.4f} active_rate={(gate > 0).float().mean().item():.4f} "
                f"conf_mean={conf.mean().item():.4f} margin_mean={margin.mean().item():.4f}")
