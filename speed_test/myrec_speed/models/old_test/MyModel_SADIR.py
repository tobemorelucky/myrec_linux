...
class MyModel(SequentialModel):
    ...
    extra_log_args = [
        ...
        "use_sadir", "lambda_bridge", "lambda_gate_cons",
        # legacy fields
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument("--lambda_bridge", type=float, default=0.02)
        parser.add_argument("--bridge_temp", type=float, default=0.20)
    ...
    def __init__(self, args, corpus):
        ...
        self.lambda_bridge = float(getattr(args, "lambda_bridge", 0.02))
        self.bridge_temp = float(getattr(args, "bridge_temp", 0.20))
        ...
        if self.use_llmemb and self.init_ckpt:
            ...
            logging.info(f"[MyModel-SADIR] Warm-start from {self.init_ckpt} (strict={bool(self.init_strict)})")
        else:
            logging.info("[MyModel-SADIR] Train from scratch")
    ...
    def forward(self, feed_dict):
        ...
        # SADIR auxiliary outputs
        if self.use_sadir:
            pos_ids = i_ids[:, 0]
            if self.emile_use_fused_itememb:
                pos_v = self.interest_extractor.get_item_emb(pos_ids)
            else:
                pos_v = self.interest_extractor.get_cf_emb(pos_ids)
            bridge_target = torch.softmax(
                self._cos_sim(interest_vectors, pos_v[:, None, :]) / self.bridge_temp, dim=-1
            )
            out_dict["bridge_target"] = bridge_target.detach()
    ...
    def loss(self, out_dict: dict):
        ...
        # SADIR bridge loss
        if self.use_sadir and ("bridge_target" in out_dict):
            target = out_dict["bridge_target"]
            pred_w = out_dict["interest_weight"]
            L_bridge = F.kl_div(torch.log(pred_w + 1e-8), target, reduction="batchmean")
            loss = loss + self._sadir_w() * (self.lambda_bridge * L_bridge)
            out_dict["loss_bridge"] = L_bridge.detach()
