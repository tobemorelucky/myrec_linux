...
class SemanticDriftRouter(nn.Module):
    ...
    - estimate sample-level confidence from coarse-interest entropy and semantic/collaborative agreement;
    - only apply strong routing / prior injection on high-confidence samples;
    - gracefully fall back to the backbone path on low-confidence samples.
    """
    def __init__(
        self,
        emb_size: int,
    ...
    def forward(
        ...
        # 1) confidence from coarse-interest entropy
        if self.K > 1:
            ent = -(coarse_weights * torch.log(coarse_weights + 1e-8)).sum(dim=-1)
            conf_w = 1.0 - ent / np.log(self.K)
        else:
            conf_w = torch.ones(B, device=device, dtype=his_fused.dtype)
        conf_w = conf_w.clamp(0.0, 1.0)

        # 2) confidence from semantic/collaborative assignment consistency
        conf_sc = self._masked_mean_per_sample(consistency, mask).clamp(0.0, 1.0)

        # 3) combined confidence with a safety floor, so sparse datasets do not collapse
        confidence = self.conf_mix * conf_w + (1.0 - self.conf_mix) * conf_sc
        confidence = (self.conf_floor + (1.0 - self.conf_floor) * confidence).clamp(0.0, 1.0)
        ...
        gate_e_raw = (p_stable + self.transition_lambda_e * p_trans).clamp(0.0, 1.0) * mask
        gate_a_raw = (p_stable + self.transition_lambda_a * p_trans).clamp(0.0, 1.0) * mask

        # confidence-aware routing: low-confidence samples softly fall back to the backbone path
        conf_bt = confidence[:, None]
        gate_e = ((1.0 - conf_bt) + conf_bt * gate_e_raw).clamp(0.0, 1.0) * mask
        gate_a = ((1.0 - conf_bt) + conf_bt * gate_a_raw).clamp(0.0, 1.0) * mask
        ...
        attn_bias = (self.prior_lambda * confidence[:, None, None]) * torch.log(prior + 1e-8)
...
class MyModel(SequentialModel):
    ...
    extra_log_args = [
        ...
        "use_sadir", "lambda_gate_cons", "sadir_conf_mix", "sadir_conf_floor",
        # legacy fields
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument("--sadir_conf_mix", type=float, default=0.5)
        parser.add_argument("--sadir_conf_floor", type=float, default=0.15)
    ...
    def __init__(self, args, corpus):
        ...
        self.sadir_conf_mix = float(getattr(args, "sadir_conf_mix", 0.5))
        self.sadir_conf_floor = float(getattr(args, "sadir_conf_floor", 0.15))
    ...
    def _define_params(self):
        ...
        self.sadir_router = SemanticDriftRouter(
            ...
            conf_mix=self.sadir_conf_mix,
            conf_floor=self.sadir_conf_floor,
        )
    ...
    def forward(self, feed_dict):
        ...
        if self.use_sadir:
            ...
            out_dict["confidence"] = aux["confidence"]
            out_dict["confidence_mask"] = valid_his.float()
    ...
    def loss(self, out_dict: dict):
        ...
        if self.use_sadir and ("gate_e" in out_dict):
            ...
            conf = out_dict.get("confidence", None)
            if conf is not None:
                weighted_mask = mask * conf[:, None]
            else:
                weighted_mask = mask
            L_gate = self._masked_mean((gate_e - gate_a) ** 2, weighted_mask)
