...
class MultiInterestExtractor(nn.Module):
    def __init__(
        ...
        self.logic_denoise_use_fused = 0
    ...
    def forward(self, history: torch.Tensor, lengths: torch.Tensor, q_vec=None):
        ...
        # -----------------------------
        # Logic-guided denoise (NO leakage at eval; outer model controls q_vec)
        # -----------------------------
        if getattr(self, "use_logic_denoise", 0) and (q_vec is not None):
            # 1) 用 CF 空间算相似度更稳（不让 llm_fuse 的尺度把 gate 冲爆）
            ...
            gate = torch.sigmoid(self.logic_denoise_alpha * sim)  # (B,L)
...
# =========================
# MyModel: PoMRec + LLMAlign + EMILE + Logic-aware Aggregation (optional)
# =========================
class MyModel(SequentialModel):
    ...
    extra_log_args = [
        ...
        "use_logic_denoise", "logic_denoise_topk",
        "use_logic_aggr", "lambda_logic_aggr",
    ]
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument("--align_on", type=str, default="pos", choices=["pos"])
        ...
        parser.add_argument("--lambda_ilr", type=float, default=0.0)
        parser.add_argument("--ipd_margin", type=float, default=0.2)
        parser.add_argument("--ilr_scale", type=float, default=10.0)
        ...
        # ---- logic-guided denoise (TRAIN only; eval disabled to avoid leakage) ----
        ...
        parser.add_argument("--logic_denoise_use_fused", type=int, default=0)

        # ---- logic-aware aggregation (optional) ----
    ...
    def __init__(self, args, corpus):
        ...
        self.lambda_ilr = float(getattr(args, "lambda_ilr", 0.0))
        self.ipd_margin = float(getattr(args, "ipd_margin", 0.2))
        self.ilr_scale = float(getattr(args, "ilr_scale", 10.0))
        self.ilr_neg_weight = float(getattr(args, "ilr_neg_weight", 1.0))
        ...
        self.logic_denoise_use_fused = int(getattr(args, "logic_denoise_use_fused", 0))

        # logic-aware aggregation (optional)
        self.use_logic_aggr = int(getattr(args, "use_logic_aggr", 1))
        self.lambda_logic_aggr = float(getattr(args, "lambda_logic_aggr", 0.3))
        ...
        self.global_step = 0
        self.logic_lambda = args.lambda_logic_aggr  # 原来的lambda
        self.logic_a = nn.Parameter(torch.tensor(5.0))  # slope
        self.logic_b = nn.Parameter(torch.tensor(0.5))  # center
    ...
    def forward(self, feed_dict):
        ...
        if self.use_logic_denoise and self.training:
            ...
            self.interest_extractor.logic_denoise_use_fused = self.logic_denoise_use_fused
            interest_vectors, distri_vectors = self.interest_extractor(history, lengths, q_vec=q_vec)
        else:
            # 完全等价 PoMRec：只 forward 一次
            self.interest_extractor.use_logic_denoise = 0
        ...
        # ------------------------------------------------------------
        # (Optional) logic-aware aggregation (candidate-aware)
        # ------------------------------------------------------------
        if getattr(self, "use_logic_aggr", 0):

            # ------------------------------------------------
            # 1) Base interest distribution (user-only)
            # ------------------------------------------------
            w = torch.softmax(base_logits, dim=-1)  # (B,K)

            # ------------------------------------------------
            # 2) Entropy-aware gate
            # ------------------------------------------------
            entropy = -(w * torch.log(w + 1e-9)).sum(dim=1)  # (B,)
            entropy_norm = entropy / math.log(w.size(1))  # normalize to [0,1]

            g = torch.sigmoid(self.logic_a * (entropy_norm - self.logic_b))  # (B,)
            lambda_eff = self.logic_lambda * g  # (B,)

            # ------------------------------------------------
            # 3) Candidate-aware residual reweight
            # ------------------------------------------------
            ...
            w_kc = w_kc / (w_kc.sum(dim=1, keepdim=True) + 1e-9)

            # ------------------------------------------------
            # 4) Aggregate
            # ------------------------------------------------
            u_c = torch.einsum("bkc,bkd->bcd", w_kc, interest_vectors)
            prediction = (u_c * i_vectors).sum(dim=-1)

            # ------------------------------------------------
            # 5) Optional debug stash (不会破坏图)
            # ------------------------------------------------
            if self.training:
        ...
        # -----------------------------
        if self.use_emile:
            w_user = torch.softmax(base_logits, dim=-1)
            user_vector_for_emile = (interest_vectors * w_user[:, :, None]).sum(dim=1)

            out_dict["emile_interest_vectors"] = interest_vectors
            out_dict["emile_user_vector"] = user_vector_for_emile
            out_dict["emile_w"] = w_user
    ...
    def loss(self, out_dict: dict):
        ...
        if self.use_emile:
            ...
            if neg_ids is not None:
                ...
                # ILR
                T = self.emile_T
                sim_multi = self._cos_sim(uv, T[None, :].expand_as(uv))
                sim_single = self._cos_sim(iv, T[None, None, :].expand_as(iv)).mean(dim=1)
                sim_neg = self._cos_sim(neg_v, T[None, :].expand_as(neg_v))

                s = self.ilr_scale
                L_ilr = (
                    self._bpr_score(sim_multi, sim_single, scale=s) +
                    self.ilr_neg_weight * self._bpr_score(sim_multi, sim_neg, scale=s)
                )

                w_em = self._emile_w()
                loss = loss + w_em * (self.lambda_ipd * L_ipd + self.lambda_ilr * L_ilr)

                out_dict["loss_ipd"] = L_ipd.detach()
                out_dict["loss_ilr"] = L_ilr.detach()
