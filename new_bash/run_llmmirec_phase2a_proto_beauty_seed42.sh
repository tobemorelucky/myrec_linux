...
# =========================================================
# LLMMIRec Phase 2A — Prototype Query (Beauty seed=42)
#
# Usage:
#   QUERY_MODE=learnable bash .../run_...sh [GPU_ID]
#   QUERY_MODE=prototype bash .../run_...sh [GPU_ID]
#   SMOKE=1 QUERY_MODE=prototype bash .../run_...sh [GPU_ID]
#
...
SEED=42
DATASET="beauty"
QUERY_MODE=${QUERY_MODE:-learnable}
SEMANTIC_RANK=512
LAMBDA_RELATION=0.01
PROTOTYPE_NUM=32

ROOT_LOG_DIR="new_log/llmmirec_phase2a_proto/${DATASET}"
ROOT_MODEL_DIR="new_model/llmmirec_phase2a_proto/${DATASET}"
...
LABEL="aspcf_proto${PROTOTYPE_NUM}_${QUERY_MODE}"
SUMMARY_FILE="${ROOT_LOG_DIR}/summary_seed${SEED}.tsv"

if [ ! -f "${SUMMARY_FILE}" ]; then
  echo -e "dataset\tmodel\titem_encoder\tquery_mode\tgate_mode\tsemantic_rank\tlambda_relation\tprototype_num\tseed\tK\tlr\tstart_time\tend_time\ttotal_seconds\tparameter_count\tbest_epoch\tbest_dev\ttest_after_training\tstatus" > "${SUMMARY_FILE}"
fi

echo "========================================================="
echo "LLMMIRec Phase 2A — ${QUERY_MODE} query ${DATASET} ${SMOKE_TAG}"
echo "GPU=${GPU}  proto_num=${PROTOTYPE_NUM}  lambda=${LAMBDA_RELATION}"
echo "========================================================="
...
# Auto-set prototype path for prototype mode
if [ "${QUERY_MODE}" = "prototype" ]; then
  ...
  if [ ! -f "${PROTO_PATH}" ]; then
    ...
    echo "  Build it first: python tools/build_llmmi_semantic_prototypes.py --dataset ${DATASET}"
    exit 1
  fi
  PROTO_ARG="--prototype_path ${PROTO_PATH}"
else
  PROTO_ARG=""
fi
...
python main.py \
  ...
  --interest_query_mode "${QUERY_MODE}" \
  ${PROTO_ARG} \
...
echo -e "${DATASET}\tLLMMIRec\taspcf\t${QUERY_MODE}\tbasic\t${SEMANTIC_RANK}\t${LAMBDA_RELATION}\t${PROTOTYPE_NUM}\t${SEED}\t4\t0.008\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
