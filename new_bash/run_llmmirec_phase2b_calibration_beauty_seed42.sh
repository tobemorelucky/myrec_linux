...
# =========================================================
# LLMMIRec Phase 2B — Collab Calibration (Beauty seed=42)
#
# Usage:
#   COLLAB_CALIBRATION=global bash .../run_...sh [GPU_ID]
#   COLLAB_CALIBRATION=prototype bash .../run_...sh [GPU_ID]
#   SMOKE=1 COLLAB_CALIBRATION=prototype bash .../run_...sh [GPU_ID]
#
...
SEED=42
DATASET="beauty"
COLLAB_CALIBRATION=${COLLAB_CALIBRATION:-global}
SEMANTIC_RANK=512
LAMBDA_RELATION=0.01
PROTOTYPE_NUM=32

ROOT_LOG_DIR="new_log/llmmirec_phase2b_calibration/${DATASET}"
ROOT_MODEL_DIR="new_model/llmmirec_phase2b_calibration/${DATASET}"
...
LABEL="proto_${COLLAB_CALIBRATION}_calib"
SUMMARY_FILE="${ROOT_LOG_DIR}/summary_seed${SEED}.tsv"

if [ ! -f "${SUMMARY_FILE}" ]; then
  echo -e "dataset\tmodel\titem_encoder\tquery_mode\tcollab_calibration\tsemantic_rank\tlambda_relation\tprototype_num\tseed\tK\tlr\tstart_time\tend_time\ttotal_seconds\tparameter_count\tbest_epoch\tbest_dev\ttest_after_training\tstatus" > "${SUMMARY_FILE}"
fi

echo "========================================================="
echo "LLMMIRec Phase 2B — collab_calibration=${COLLAB_CALIBRATION} ${SMOKE_TAG}"
echo "GPU=${GPU}  ${MODEL_LABEL}"
echo "========================================================="
...
python main.py \
  ...
  --collab_calibration "${COLLAB_CALIBRATION}" \
...
echo -e "${DATASET}\tLLMMIRecCHIR\taspcf\tprototype\t${COLLAB_CALIBRATION}\t${SEMANTIC_RANK}\t${LAMBDA_RELATION}\t${PROTOTYPE_NUM}\t${SEED}\t4\t0.008\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
