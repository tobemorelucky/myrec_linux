...
# =========================================================
# LLMMIRec ASPCF Phase 2 (final) — Beauty baseline
#
# Same seed/lr/batch/ASPCF/relation/eval config as CAISD Phase 2.
# The ONLY methodological difference vs CAISD is Chapter 4's
# interest semantic teacher + semantic distillation.
#
# Usage:
#   bash new_bash/run_llmmirec_aspcf_phase2_beauty.sh GPU SEED
#   e.g. bash new_bash/run_llmmirec_aspcf_phase2_beauty.sh 0 42
# =========================================================
...
ROOT_LOG_DIR="new_log/llmmirec_aspcf_phase2/${DATASET}/seed${SEED}"
ROOT_MODEL_DIR="new_model/llmmirec_aspcf_phase2/${DATASET}/seed${SEED}"
SUMMARY_FILE="new_log/llmmirec_aspcf_phase2/${DATASET}/summary.tsv"
...
START_TS=$(date +%s)
echo "[START] ${START_TIME} | ASPCF ${DATASET} seed=${SEED} gpu=${GPU}"
...
echo -e "${DATASET}\t${MODEL_NAME}\t${SEED}\tnone\t0\t${LR}\t${BATCH_SIZE}\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
