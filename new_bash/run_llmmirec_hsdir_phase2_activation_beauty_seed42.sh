...
# =========================================================
# LLMMIRec HSDIR Phase 2 — Support-Confidence Calibration (Beauty seed=42)
#
# Usage:
#   AGGREGATION_MODE=support_confidence SUPPORT_BETA=1.0 bash .../run_...sh [GPU_ID]
#   AGGREGATION_MODE=base bash .../run_...sh [GPU_ID]
#   SMOKE=1 AGGREGATION_MODE=support_confidence bash .../run_...sh [GPU_ID]
#
...
SEED=42
DATASET="beauty"
AGGREGATION_MODE=${AGGREGATION_MODE:-support_confidence}
SUPPORT_BETA=${SUPPORT_BETA:-1.0}
HSR_TEACHER_MODE=hierarchical
LAMBDA_HSR=0.01
LAMBDA_RELATION=0.01
SEMANTIC_RANK=512

ROOT_LOG_DIR="new_log/llmmirec_hsdir_phase2_activation/${DATASET}"
ROOT_MODEL_DIR="new_model/llmmirec_hsdir_phase2_activation/${DATASET}"
...
LABEL="hsdir_${HSR_TEACHER_MODE}_lh${LAMBDA_HSR}_${AGGREGATION_MODE}_b${SUPPORT_BETA}"
SUMMARY_FILE="${ROOT_LOG_DIR}/summary_seed${SEED}.tsv"

if [ ! -f "${SUMMARY_FILE}" ]; then
  echo -e "dataset\tmodel\tteacher_mode\tlambda_hsr\tagg_mode\tsupport_beta\tseed\tK\tlr\tstart_time\tend_time\ttotal_seconds\tparameter_count\tbest_epoch\tbest_dev\ttest_after_training\tstatus" > "${SUMMARY_FILE}"
fi

echo "========================================================="
echo "LLMMIRec HSDIR Phase 2 — ${AGGREGATION_MODE} beta=${SUPPORT_BETA} ${SMOKE_TAG}"
echo "GPU=${GPU}"
echo "========================================================="
...
echo -e "${DATASET}\tLLMMIRecHSDIR\t${HSR_TEACHER_MODE}\t${LAMBDA_HSR}\t${AGGREGATION_MODE}\t${SUPPORT_BETA}\t${SEED}\t4\t0.008\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\tOK" >> "${SUMMARY_FILE}"
