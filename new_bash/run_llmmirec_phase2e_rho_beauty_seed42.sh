...
# =========================================================
# LLMMIRec Phase 2E — Routing Rho Ablation (Beauty seed=42)
#
# Usage:
#   RHO_MODE=fixed RHO_VALUE=0.5 bash .../run_...sh [GPU_ID]
#   RHO_MODE=fixed RHO_VALUE=1.0 bash .../run_...sh [GPU_ID]
#   RHO_MODE=fixed RHO_VALUE=0.0 bash .../run_...sh [GPU_ID]
#   RHO_MODE=learned bash .../run_...sh [GPU_ID]   (baseline)
#   SMOKE=1 RHO_MODE=fixed RHO_VALUE=0.5 bash .../run_...sh [GPU_ID]
#
...
SEED=42
DATASET="beauty"
RHO_MODE=${RHO_MODE:-fixed}
RHO_VALUE=${RHO_VALUE:-0.5}
SEMANTIC_RANK=512
LAMBDA_RELATION=0.01
PROTOTYPE_NUM=32

ROOT_LOG_DIR="new_log/llmmirec_phase2e_rho/${DATASET}"
ROOT_MODEL_DIR="new_model/llmmirec_phase2e_rho/${DATASET}"
...
LABEL="rho_${RHO_MODE}_${RHO_VALUE}"
SUMMARY_FILE="${ROOT_LOG_DIR}/summary_seed${SEED}.tsv"

if [ ! -f "${SUMMARY_FILE}" ]; then
  echo -e "dataset\tmodel\trho_mode\trho_value\tsemantic_rank\tlambda_relation\tprototype_num\tseed\tK\tlr\tstart_time\tend_time\ttotal_seconds\tparameter_count\tbest_epoch\tbest_dev\ttest_after_training\tstatus" > "${SUMMARY_FILE}"
fi

echo "========================================================="
echo "LLMMIRec Phase 2E — rho=${RHO_MODE}/${RHO_VALUE} ${SMOKE_TAG}"
echo "GPU=${GPU}"
echo "========================================================="
...
python main.py \
  ...
  --routing_rho_mode "${RHO_MODE}" \
  --routing_rho_value "${RHO_VALUE}" \
...
echo -e "${DATASET}\tLLMMIRecCHIR\t${RHO_MODE}\t${RHO_VALUE}\t${SEMANTIC_RANK}\t${LAMBDA_RELATION}\t${PROTOTYPE_NUM}\t${SEED}\t4\t0.008\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
