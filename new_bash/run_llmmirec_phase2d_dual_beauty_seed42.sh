...
# =========================================================
# LLMMIRec Phase 2D — Dual-View Routing (Beauty seed=42)
#
# Usage:
#   bash new_bash/run_llmmirec_phase2d_dual_beauty_seed42.sh [GPU_ID]
...
if [ ! -f "${SUMMARY_FILE}" ]; then
  echo -e "dataset\tmodel\titem_encoder\tquery_mode\tcollab_calibration\trouting_mode\tsemantic_rank\tlambda_relation\tprototype_num\tseed\tK\tlr\tstart_time\tend_time\ttotal_seconds\tparameter_count\tbest_epoch\tbest_dev\ttest_after_training\tstatus" > "${SUMMARY_FILE}"
fi
...
echo -e "${DATASET}\tLLMMIRecCHIR\taspcf\tprototype\tprototype\tdual\t${SEMANTIC_RANK}\t${LAMBDA_RELATION}\t${PROTOTYPE_NUM}\t${SEED}\t4\t0.008\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
