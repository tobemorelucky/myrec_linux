...
ROOT_LOG_DIR="new_log/hmif_latest"
ROOT_MODEL_DIR="new_model/hmif_latest"
...
run_one() {
  ...
  echo ""
  echo "========================================================="
  echo "[START] ${DATASET} | MyModelHMIF | ${VARIANT}"
  echo "GPU=${GPU}"
  echo "init_ckpt=${INIT_CKPT}"
  echo "eta=${ETA}"
  echo "hmif_temp=${HMIF_TEMP}"
  echo "log=${LOG_FILE}"
  echo "========================================================="

  python main.py \
    --model_name MyModelHMIF \
    --dataset "${DATASET}" \
    --path ./data/ \
    --gpu "${GPU}" \
    --random_seed "${SEED}" \
  ...
  echo "========================================================="
  echo "[DONE] ${DATASET} | MyModelHMIF | ${VARIANT}"
  echo "${BEST_DEV}"
  echo "${TEST_AFTER}"
  echo "========================================================="

  echo -e "${DATASET}\tMyModelHMIF\t${VARIANT}\t${SEED}\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${BEST_DEV}\t${TEST_AFTER}" >> "${SUMMARY_FILE}"
}

run_one \
  "beauty" \
  "beauty_HMIF_eta010_temp020_seed42" \
  0.002 \
  0.1 \
  0.2 \
  "${BEAUTY_LLM}" \
...
run_one \
  "ml-1m" \
  "ml1m_HMIF_eta005_temp020_seed42" \
  0.001 \
  0.08 \
  0.3 \
  "${ML1M_LLM}" \
...
echo ""
echo "========================================================="
echo "Latest HMIF runs finished."
echo "Summary:"
