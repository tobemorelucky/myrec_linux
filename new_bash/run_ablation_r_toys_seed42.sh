...
ROOT_LOG_DIR="new_log/ablation_r/toys"
ROOT_MODEL_DIR="new_model/ablation_r/toys"
mkdir -p "${ROOT_LOG_DIR}" "${ROOT_MODEL_DIR}"

LLM_PATH="./data/toys/handled/llm_table_pca1536.pkl"
SRS_PATH="./data/toys/handled/itm_emb_pomrec.pkl"
...
# Toys best: lr=0.001, gamma=0.05, tau=0.5, lamb=3.8
# IPD: lambda=0.05, margin=0.10, warmup=20000
# LGD: alpha=10, b=0.3, topk=10, warmup=50000
R_VALUES=(0 0.05 0.10 0.15 0.20)

run_one() {
  local R=$1
  local VARIANT="toys_r$(echo $R | sed 's/\.//g')_seed${SEED}"
  ...
  python main.py \
    --model_name MyModel --dataset toys --path ./data/ --gpu "${GPU}" \
    --random_seed "${SEED}" --load 0 \
    --emb_size 64 --attn_size 8 --K 3 --prompt_num 4 --n_layers 1 --lamb 3.8 --history_max 20 \
    --use_llmemb 1 --llm_fuse 1 --llm_emb_path "${LLM_PATH}" --srs_emb_path "${SRS_PATH}" \
    --gamma_init 0.05 --gamma_trainable 0 --alpha 0.001 --tau 0.5 --rat_alpha_warmup_steps 5000 \
    --init_ckpt "${INIT_CKPT}" --init_strict 0 \
    --use_emile 1 --lambda_ipd 0.05 --ipd_margin 0.10 --emile_use_fused_itememb 0 --emile_warmup_steps 20000 \
    --use_logic_denoise 1 --logic_denoise_alpha 10 --logic_denoise_b 0.3 --logic_denoise_topk 10 \
  ...
  echo -e "toys\tMyModel\t${VARIANT}\t${SEED}\t${R}\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${BEST_DEV}\t${TEST_AFTER}" >> "${SUMMARY_FILE}"
}
...
echo ""
echo "========================================================="
echo "Toys r-ablation finished. Summary:"
