...
# =========================================================
# LLMMIRec Phase 1 — ASPCF Beauty seed=42
#
# Usage:
#   bash new_bash/run_llmmirec_phase1_aspcf_beauty_seed42.sh [GPU_ID]
#   LAMBDA_RELATION=0.01 SEMANTIC_RANK=256 bash .../run_...sh [GPU_ID]
#   SMOKE=1 bash .../run_...sh [GPU_ID]
#
# GPU control: $1 (default 0) is passed as --gpu.
# For GPU 1:   bash .../run_...sh 1
# =========================================================
...
ROOT_LOG_DIR="new_log/llmmirec_phase1_aspcf/${DATASET}"
ROOT_MODEL_DIR="new_model/llmmirec_phase1_aspcf/${DATASET}"
...
LABEL="aspcf_sr${SEMANTIC_RANK}_lr${LAMBDA_RELATION}"
...
echo "========================================================="
echo "LLMMIRec Phase 1 — ASPCF ${DATASET} seed=${SEED} ${SMOKE_TAG}"
echo "GPU=${GPU}  EPOCH=${EPOCH}  EARLY_STOP=${EARLY_STOP}"
echo "semantic_rank=${SEMANTIC_RANK}  lambda_relation=${LAMBDA_RELATION}"
echo "========================================================="
...
echo "========================================================="
echo "[DONE] ${LABEL} seed=${SEED} ${SMOKE_TAG}"
echo "start=${START_TIME}  end=${END_TIME}  total_seconds=${TOTAL_SECONDS}"
echo "${BEST_DEV}"
echo "${TEST_AFTER}"
echo "========================================================="

echo -e "${DATASET}\tLLMMIRec\taspcf\t${SEMANTIC_RANK}\t${LAMBDA_RELATION}\t${SEED}\t4\t0.001\t${START_TIME}\t${END_TIME}\t${TOTAL_SECONDS}\t${PARAM_COUNT}\t${BEST_EPOCH}\t${BEST_DEV}\t${TEST_AFTER}\t${STATUS}" >> "${SUMMARY_FILE}"
