...
# 1) 弱 bias + extractor-side SC
launch_one "ml1m_pasv3_bE003_bA002_scE001_s42" \
  --pas_bias_e 0.03 \
  --pas_bias_a 0.02 \
  --lambda_sampler_sc_e 0.001 \
  --lambda_sampler_sc_a 0.0

# 2) 推荐配置：中等偏弱 bias + extractor-side SC
launch_one "ml1m_pasv3_bE005_bA003_scE001_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03 \
  --lambda_sampler_sc_e 0.001 \
  --lambda_sampler_sc_a 0.0

# 3) 稍强 bias + extractor-side SC
launch_one "ml1m_pasv3_bE008_bA005_scE001_s42" \
  --pas_bias_e 0.08 \
  --pas_bias_a 0.05 \
  --lambda_sampler_sc_e 0.001 \
  --lambda_sampler_sc_a 0.0

# 4) 推荐 bias + 较弱 extractor-side SC
launch_one "ml1m_pasv3_bE005_bA003_scE0005_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03 \
...
# 5) 推荐 bias + 去掉 SC
launch_one "ml1m_pasv3_bE005_bA003_scE0000_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03 \
  --lambda_sampler_sc_e 0.0 \
  --lambda_sampler_sc_a 0.0
...
wait
echo "ML-1M PAS-v3 small sweep finished. Logs in ${LOGDIR}"