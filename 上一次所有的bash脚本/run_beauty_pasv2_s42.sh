...
# 1) Beauty 保守轻强度
launch_one "beauty_pasv2_bE003_bA002_s42" \
  --pas_bias_e 0.03 \
  --pas_bias_a 0.02

# 2) Beauty 推荐强度
launch_one "beauty_pasv2_bE005_bA003_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03

# 3) Beauty 稍强
launch_one "beauty_pasv2_bE008_bA005_s42" \
  --pas_bias_e 0.08 \
  --pas_bias_a 0.05

# 4) 推荐强度 + 较弱辅助损失
launch_one "beauty_pasv2_bE005_bA003_lowaux_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03 \
...
# 5) 推荐强度 + 去掉语义协同一致性辅助损失
launch_one "beauty_pasv2_bE005_bA003_nosc_s42" \
  --pas_bias_e 0.05 \
  --pas_bias_a 0.03 \
  --lambda_sampler_sc 0.0

wait
echo "Beauty PAS-v2 small sweep finished. Logs in ${LOGDIR}"