...
# 新 gate：扫 b（4组），固定 topk=5
#B_LIST=(0.0 0.1 0.2 0.3)
B_LIST=(0.1 0.3)
TOPK=5

# 建议把 alpha 提高（否则 sigmoid 太平）
LGD_ALPHA=8.0
LGD_R=0.15
LGD_WARMUP=20000
...
launch_one () {
  ...
  local b_s="${b//./}"
  local tag="beauty_llm_ipd_lgdB_a8_b${b_s}_top${TOPK}_r015_s${seed}"
  ...
  nohup python main.py \
    ...
    --logic_denoise_topk ${TOPK} --logic_denoise_warmup_steps ${LGD_WARMUP} \
...
wait
echo "Beauty LGD b-sweep finished. Logs in ${LOGDIR}"