...
class BaseRunner(object):
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument('--align_warmup', type=int, default=5)
        parser.add_argument('--align_ramp', type=int, default=15)
        return parser
    ...
    def __init__(self, args):
        ...
        self.align_warmup = int(getattr(args, "align_warmup", 5))
        self.align_ramp = int(getattr(args, "align_ramp", 15))
    ...
    def train(self, data_dict: Dict[str, BaseModel.Dataset]):
        ...
        try:
            for epoch in range(self.epoch):
                ...
                model._cur_epoch = epoch + 1
                model.align_warmup = getattr(model, "align_warmup", 5)
                model.align_ramp = getattr(model, "align_ramp", 15)
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        ...
        model.cur_epoch = epoch  # ✅ 让模型知道当前 epoch
        if hasattr(model, "actions_before_epoch"):
            model.actions_before_epoch()  # ✅ 让模型在每个 epoch 开始时调整冻结策略/alpha
